...
class imgModule extends mod_1.mod {
    constructor() {
        super(...arguments);
    ...
    getIconID(key) {
        let k = key;
        let x = component_1.iconID[k];
        if (!x)
            return [component_1.iconID.crash, false];
        return [x, true];
    }
    evaluate(cmd, args, option, raw) {
        ...
        if (isInIconMode) {
            ...
            return [
                new component_1.iconComponent(iconID, isCorrect ? undefined : `Wrong iconID, received = ${str}`, cmd, raw)
            ];
        }
        else {
            return [
                new component_1.imageComponent(str, undefined, cmd, raw)
            ];
        }
    }
}
